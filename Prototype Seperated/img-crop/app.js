...
var imageCrop = function (imageTarget, requiredWidth, requiredHeight) {
  ...
  function init() {
    origSrc.onload = function () {
      ...
      // console.log(imageScale, origSrc, $originalImage);
  ...
  function resizing(e) {
    ...
    if (SE) {
      console.log(eventState);
      // MOUSE.X : ACC TO SCREEN
      // EVENTsTATE.CONTAINERlEFT: CONTAINER KE HISSAB SE
      // ORIGINAL IMAGE IS CONTAINER WALI IMAGE
    ...
    if (allowResize) {
      // Over top boundary
      if (top < 0) {
        console.log(originalHeight, originalOffset.top);

        //originalOffset.top relative to container
